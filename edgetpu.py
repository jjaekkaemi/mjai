...
class EdgeTPUModel:
        ...
        if len(det):
            ...
            for *xyxy, conf, cls in reversed(det):
                if save_img:  # Add bbox to image
                    ...
                    print(output_image.data.contiguous)
            ...
            if save_img:
              output_count+=1
              cv2.imwrite(f"detect2/{output_count}_defect.jpg", output_image)
    ...
    def process_predictions_crop(self, det, output_image, pad, output_path="detection.jpg", save_img=True, save_txt=True, hide_labels=False, hide_conf=False):
        """
        Process predictions and optionally output an image with annotations
        """
        ob_xyxy_list2 = []
        if len(det):
            ...
            ob_xywh_list = []
            for *xyxy, conf, cls in reversed(det):
                gn = torch.tensor(output_image.shape)[[1,0,1,0]]
                ...
                b_xyxy = torch.tensor(xyxy).view(-1, 4)
                b = self.xyxy2xywh(b_xyxy)

                b[:, 2:] = b[:, 2:]*gain+pad
                b_xyxy = self.xywh2xyxy(b).long()
                self.clip_boxes(b_xyxy, output_image.shape)
                crop_img0s = output_image[int(b_xyxy[0, 1]):int(b_xyxy[0, 3]), int(b_xyxy[0, 0]):int(b_xyxy[0, 2]), ::(1 if True else -1)]
                ob_xyxy_list.append(crop_img0s)
                ob_xywh_list.append(xywh)

                self.count+=1
                cv2.imwrite(f"detect/{self.count}_output.jpg", crop_img0s)
