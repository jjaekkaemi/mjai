...
def run(
    ...
    while True:
        try:
            ...
            dtime_ori = os.path.join(save_img_dir+"/ori", dtime)
            ret = cam.MV_CC_GetOneFrameTimeout(data_buf, nPayloadSize, stFrameInfo, 1000)
            if ret == 0:
                ...
                im = letterbox__(frame__, 416, 32, True)[0]  # padded resize
            ...
            for i, det in enumerate(pred):  # per image
                ...
                if len(det):
                    ...
                    for *xyxy, conf, cls in reversed(det):
                        ...
                        crop_im__ = letterbox__(crop_img0s, 416, stride=32, auto=True)[0]
                        # # Convert
                        ...
                        crop_pred = non_max_suppression(crop_pred, 0.50, iou_thres, classes, agnostic_nms, max_det=max_det)
                        ...
                        for i, det in enumerate(crop_pred):
                            ...
                            if defect_flag :
                                file_s += f"===================={dtime_defect}{board_count}.jpg====================\n"
                                defect_file = open ("/home/dtriple/test/230314.txt", "a")
                                defect_file.write(file_s)
                                defect_file.close()
                                cv2.imwrite(dtime_defect+f"{board_count}.jpg", crop_im0)
                                board_count+=1
                                cv2.imshow("test", crop_im0)
                                cv2.waitKey(1) 
